...
stream=open("stations.dat",mode="r")
state1='from'
stns=stream.readlines()
for i in stns:
   components=i.split(",")
   ##print(components[1]+components[2])
   code=components[2].split()[0].lower()
   code+='=metrorail2.Station(\" '
   code+=components[1]
   code+=" \" ,\" "
   code+=components[2].split()[0]+"\")"
   ##print(code)



   exec(code)
   ##allStations.append(((components[2].split()[0].lower())))
